...
2. `public.attendment_ai_interaction` → representa **eventos do agente/IA** dentro do atendimento (respostas de modelo, **ações internas** e **chamadas de ferramentas**).
...
- `id` (uuid) - pode omitir para usar gen_random_uuid().
...
- `status` (public.attendment_status) - ex.: 'active', 'ended'.
...
## 2) public.attendment_ai_interaction
...
- `public.ai_interaction_kind`: 'model_response' | 'action' | 'tool_call'
- `public.ai_interaction_status`: 'ok' | 'error' | 'partial' (status padronizado do evento)
- `public.ai_action_type`:
  - REQUEUE | ARCHIVE_CONVERSATION | FINISH_CONVERSATION | TRANSFER_CONVERSATION | CHANGE_DEPARTMENT | CHANGE_WORKFLOW | CHANGE_WORKFLOW_STAGE | CHANGE_TABULATION | ADD_TAG | REMOVE_TAG | ADD_NOTE | SEND_FILE | SEND_IMAGE | SEND_AUDIO | UPDATE_CONTACT | TRANSFER_TO_FLOW
...
- Não enviar `started_at` e `ended_at`. Isso vai ser preenchido pelo banco.
...
2. Durante a conversa:
   ...
   - grava **tool_call** quando a IA chamar integração externa.
